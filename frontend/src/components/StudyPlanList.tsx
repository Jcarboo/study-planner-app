...
export default function StudyPlanList({ refresh }: StudyPlanListProps) {
  ...
  const fetchPlans = async () => {
    try {
      const res = await axios.get('http://study-planner-backend-xjed.onrender.com/study/all', {
  ...
  const deletePlan = async (planId: string) => {
    ...
    if (!confirm) return;
    try {
      await axios.delete(`http://study-planner-backend-xjed.onrender.com/study/${planId}`, {
        withCredentials: true,
      });
      fetchPlans();
    } catch (err) {
  ...
  const deleteTask = async (planId: string, taskName: string) => {
    try {
      await axios.post(
        `http://study-planner-backend-xjed.onrender.com/study/${planId}/delete-task`,
        { task_name: taskName },
        { withCredentials: true }
      );
      fetchPlans();
  ...
  const addTask = async (planId: string) => {
    try {
      await axios.post(
        `http://study-planner-backend-xjed.onrender.com/study/${planId}/add-task`,
  ...
  const toggleTaskDone = async (
    ...
    try {
      await axios.post(
        `http://study-planner-backend-xjed.onrender.com/study/${planId}/toggle-task`,
  ...
  const toggleFeatured = async (planId: string, current: boolean | undefined) => {
    try {
      await axios.post(
        `http://study-planner-backend-xjed.onrender.com/study/${planId}/feature`,
        { featured: !current },
        { withCredentials: true }
      );
      fetchPlans();
